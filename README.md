...
and `Fortify_AllowAllocate`, which are used by Christopher Bazley's test
suites. Setting the allocation-count limit resets its count; `ULONG_MAX`
disables that form of fault injection. `Fortify_AllowAllocate` applies the
same percentage and allocation-count fault injection as an intercepted
allocation without creating a dummy Fortify memory block. When
